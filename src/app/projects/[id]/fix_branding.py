...
# 1. Populate Global Tab
new_global_content = r'''                        {/* CORPORATE IDENTITY CONTROLS (Moved from Config) */}
...
# Search for placeholder in brandingInnerTab === 'global' block
pattern_global = re.compile(r'\{brandingInnerTab === \'global\' && \(.*?\)\}', re.DOTALL)
global_replacement = r'''{brandingInnerTab === 'global' && (
                      <div className="space-y-12">
''' + new_global_content + r'''                      </div>
                    )}'''

if pattern_global.search(content):
    content = pattern_global.sub(global_replacement, content)
    print("Global tab replaced")
else:
    print("Global tab pattern NOT found")
